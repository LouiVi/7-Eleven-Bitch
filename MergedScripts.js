...
function About()
{
    ...
    var img = app.CreateImage( "Img/7 Eleven Bitch.png", 0.2 );
...
function File( path, layContent )
{
    var self = this;
    var name = "";
    var dfltImage = "Img/7 Eleven Bitch.png";
    var dfltText = "";
    var img2 =	new Img();
    ...
    this.Load = function()
    {
        ...
        if( json ) 
        {
            ...
            var dfltName = name ;
            img2.SetImage( data.image ? data.image : dfltImage, img2.GetWidth() );
            img2.imageFile =  data.image ? data.image : dfltImage;
    ...
    this.Clear = function()
    {
//        img2.SetImage( dfltImage );
       // img2.imageFile =  dfltImage;
    ...
    this.OnImageChoose = function( file )
    {
        ...
        img2.SetImage( imageFile, 0.2,-1 );
        img2.imageFile = imageFile;
    ...
    this.btnImage_OnTouch = function()
    {
        app.ChooseFile( "Choose Image", "image/*", self.OnImageChoose );
        	img2.SetName( prompt("Name: ","Cricota" ));
        alert(img2.GetName());
    }
    ...
    //Create image.
	var img2 = app.CreateImage( dfltImage, 0.2, -1 );

//img2.SetImage( dfltImage);

	layHoriz.AddChild( img2 );
...
function Home( path, layContent )
{
    ...
    {
        if( show ) lay.Animate("FadeIn")/*, i = setInterval(self.Swap, 250)*/
        else lay.Animate( "FadeOut" );
    }
...
*/
    ...
    lay.AddChild( player );
    player.SetFile( "Misc/7 Eleven Bitch.mp4");
    player.SetOnReady( ()=>{player.Play();} );
    player.SetOnComplete( ()=>{player.Play();} );
    //Add a logo.
	var img = app.CreateImage( null, 0.75 );
//	img.Image = "Img/gettyimages-117012363-612x612-removebg-preview.png";
	lay.AddChild( img );
